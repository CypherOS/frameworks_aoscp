...
public class Chip extends RelativeLayout {
    ...
    private void initImageView() {
        ...
        if (mChipIcon != null && getBitmapFromDrawable() != null) {
    ...
    private void initTextView() {
        ...
        mChipTextView.setId(Bits.TEXT_RESID);
    ...
    private void initBackgroundColor() {
		int cornerRadius = (int) getResources().getDimension(R.dimen.chip_height) / 2;

        GradientDrawable bg = new GradientDrawable();
        bg.setShape(GradientDrawable.RECTANGLE);
        bg.setCornerRadii(new float[]{cornerRadius, cornerRadius, cornerRadius, cornerRadius,
	...
	private Bitmap getBitmapFromDrawable() {
		try {
		    Bitmap bitmap;
			int size = (int) getResources().getDimension(R.dimen.chip_height);
		    bitmap = Bitmap.createBitmap(size, size, Bitmap.Config.ARGB_8888);
			Canvas canvas = new Canvas(bitmap);
            mChipIcon.setBounds(0, 0, canvas.getWidth(), canvas.getHeight());
